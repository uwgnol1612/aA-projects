...
describe Dessert do
  ...
  subject(:dessert) {Dessert.new("Muffin", 50, chef)}
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect {Dessert.new("Muffin", "tones", chef)}.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("The Great Chef")
      expect(dessert.serve).to eq("The Great Chef has made 50 Muffins!")
    end  
  end
