...
function maxNonAdjacentSum(nums) {
    ...
    for (let i = 1; i < table.length; i++) {
        let secondLeft = table[i - 2] === undefined ? 0 : table[i - 2];
        let total = secondLeft + nums[i];
        let firstLeft = table[i - 1];
        table[i] = Math.max(total, firstLeft);
    }
    return table[table.length - 1];
...
function maxNonAdjacentSum(nums, memo={}) {
    ...
    let total = nums[0] + maxNonAdjacentSum(nums.slice(2), memo);
    let firstLeft = maxNonAdjacentSum(nums.slice(1), memo);
    memo[nums.length] = Math.max(total, firstLeft);
